...
public class Player : NetworkBehaviour
{
    ...
    void CmdGraze()
    {
        ...
        RpcShowMessage(true, "", 0); // clears the text and timer messages
    }
    ...
    void RpcDrawACard(string cardName)
    {
        ...
        // set sorting order of the drawn card, re-enable previews and dragging
        s.OnComplete(() =>
        {
            ...
            drawnCard.transform.GetChild(0).GetComponent<Canvas>().sortingOrder = (-(0 + 1) * 10);
        ...
        // set new sorting orders for the other cards
        for (int i = 1; i < numberOfCardsInHand + 1; i++) handVisual.GetComponent<SameDistanceChildren>().Children[i].transform.GetChild(0).GetChild(0).GetComponent<Canvas>().sortingOrder = (-(i + 1) * 10);
    }
